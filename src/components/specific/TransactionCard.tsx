...
import { WalletTransaction } from "@/src/types/api.types";
...
export const TransactionCard = ({ transaction, walletAddress }: TransactionCardProps) => {
  ...
  const amount = parseFloat(transaction.value) / 1e18; // Convert wei to MATIC
  ...
  return (
    <View style={[
      styles.container,
      {
        borderLeftColor: isIncoming ? colors.success : colors.error,
        backgroundColor: isIncoming ? colors.background.success : colors.background.error
      }
    ]}>
